...
const updateMessage = (input, response) => {
  ...
  if (response.intents && response.intents[0]) {
    ...
    if (intent.confidence >= 0.75) {
      responseText = 'Eu entendi que sua intensão era ' + intent.intent;
    } else if (intent.confidence >= 0.5) {
      responseText = 'Eu acredito que sua intensão era ' + intent.intent;
    } else {
      responseText = 'Eu não entendi o que você quiz dizer';
    }
  }
